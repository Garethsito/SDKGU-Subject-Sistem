...
import { 
  Controller, Get, Post, Delete, Param, Body, HttpException, 
...
export class UpdateGradeDto {
  ...
  grade: string;
  sessionId?: number;
  status?: string;      // 🔥 CORREGIDO: antes NO existía
}
...
export class GradesController {
  ...
  // 📌 GET - Recomendaciones globales
  @Get('recommendations')
  async getGlobalRecommendations() {
    try {
      const rec = await this.gradesService.getGlobalRecommendations();
      return this.serializeBigInt(rec);
    } catch (error) {
  ...
  async updateGrade(
    ...
    try {
      const id = BigInt(studentId);
      const { courseCode, grade, status, sessionId } = dto;

      if (!courseCode || !grade) {
        throw new HttpException(
          'courseCode y grade son requeridos',
          HttpStatus.BAD_REQUEST
        );
      }

      const res = await this.gradesService.updateGrade(
        id,
        courseCode,
        grade,
        status ?? "Completed",
  ...
  // 📌 POST - Actualización masiva (batch)
  @Post('students/:studentId/grades/batch')
  async batchUpdateGrades(
    @Param('studentId') studentId: string,
    @Body() batchDto: BatchUpdateGradesDto
  ): Promise<BatchUpdateResult> {
    try {
      const id = BigInt(studentId);
      if (!Array.isArray(batchDto.grades)) {
        throw new HttpException('grades debe ser un array', HttpStatus.BAD_REQUEST);
      }

      const res = await this.gradesService.batchUpdateGrades(id, batchDto.grades);
      return this.serializeBigInt(res);
    } catch (error) {
  ...
  async importGrades(@UploadedFile() file: Express.Multer.File) {
    try {
      ...
      if (!rows.length) {
        throw new HttpException('Excel vacío', HttpStatus.BAD_REQUEST);
      }

      const results: any[] = [];

      for (const row of rows) {
        const { studentId, courseCode, grade, status, sessionId } = row;
